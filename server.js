...
async function fetchImageToDataURI(url) {
  ...
  if (!isImageContentType(ct)) {
    let sample = "";
    try {
      sample = Buffer.from(resp.data).toString("utf8").slice(0, 200);
    } catch {}
  ...
  switch ((a.domain || "").toLowerCase()) {
    ...
    case "phone_parts":
      return add(
        ...
        a.brand,
        a.model,
        flat(a.compatibility).join(" "),
        flat(a.features).join(" "),
        a.keywords
      );
    case "auto_parts":
      return add(
  ...
  try {
    ...
    if (audioBase64) {
      ...
      if (!isAudioVideo) {
        let sample = "";
        try {
          sample = Buffer.from(r.data).toString("utf8").slice(0, 160);
        } catch {}
...
app.post("/vision/analyze", async (req, res) => {
  try {
    ...
    const resp = await axios.post("https://api.openai.com/v1/chat/completions", payload1, {
      headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, "Content-Type": "application/json" },
    });
    let content = resp?.data?.choices?.[0]?.message?.content || "{}";
    let attrs;
    try {
      attrs = JSON.parse(content);
    } catch {
      attrs = {};
    }
    ...
    if (!attrs || !attrs.domain) {
      ...
      try {
        attrs = JSON.parse(r2?.data?.choices?.[0]?.message?.content || "{}");
      } catch {
        attrs = { domain: "other", raw: content || "" };
      }
    }
    ...
    return res.status(500).json({ error: "OpenAI error", details: details || e.message });
  }
});
...
app.post("/catalog/search", async (req, res) => {
  try {
    ...
    if (!process.env.SHOPIFY_STORE_DOMAIN || !process.env.SHOPIFY_STOREFRONT_TOKEN) {
      return res.json({
        results: [],
        note: "Faltan credenciales Shopify (SHOPIFY_STORE_DOMAIN/STOREFRONT_TOKEN).",
        query,
      });
    }
    const gql = {
    ...
    return res.json({ results: items, query });
  } catch (e) {
    return res.status(500).json({ error: "Error buscando en catálogo", details: e?.response?.data || e.message });
  }
});
  ...
  try {
    ...
    if (process.env.SHOPIFY_STORE_DOMAIN && process.env.SHOPIFY_STOREFRONT_TOKEN && query) {
      const { data: s } = await axios.post(
        `${req.protocol}://${req.get("host")}/catalog/search`,
        { query },
        { headers: { "Content-Type": "application/json" } }
      );
      results = s?.results || [];
    }
    return res.json({ attributes: attrs, query, results });
  } catch (e) {
    return res.status(500).json({ error: "Error en búsqueda por imagen", details: e?.response?.data || e.message });
  }
});
